...
source("core_casanovafit.R")
source("CASANOVA_quadratic_programming.R")
library(bazar)


casanovafit = function ( y, xlinear, xshrink, AIC = TRUE, mBICconstant = 25, default.lambdaseq = TRUE, default.length = 150,
                         lambdaseq = NULL, interceptxlinear = FALSE, BICearlystop = TRUE, BICterminate = 50,
  ...
  max.lambda = 10 * null.deviance * pshrink # This is a trial value at the start, don't read too much into this
  # if ( is.null(prev.coefficients) == F ) max.lambda = max.lambda / sqrt(sum(prev.coefficients^2))
  lambdaseq = c(0)
  if ( default.lambdaseq == TRUE ) {
    # lambdaseq = c(0, exp(seq(log(0.001), log(max.lambda), length = default.length - 1)))
    lambdaseq = seq(from = 0, to = sqrt(max.lambda), length = default.length)^2
  }
  print(max.lambda)
  ...
  if ( K > 1 ) {
    ...
    for ( k in 1:K ) {
      ...
      L = CASANOVA_objectmaker(yfold, xlinearfold, xshrinkfold, adaptive, prev.coefficients)

      casmodel = osqp(L$Dmat, L$dvec, L$Amat, L$lvec, L$uvec, pars = osqpSettings(eps_abs = 1e-8, eps_rel = 1e-9, max_iter = 2e5, verbose = FALSE))

      # For the gaussian regression version we don't require a core wrapper, since we have all the results already surely?
      cvsolution = core_casanovafit(casmodel, yfold, xlinearfold, xshrinkfold, L, k, AIC, mBICconstant, lambdaseq, BICearlystop, BICterminate )
    ...
    L = CASANOVA_objectmaker(y, xlinear, xshrink, adaptive, prev.coefficients)
    casmodel = osqp(L$Dmat, L$dvec, L$Amat, L$lvec, L$uvec, pars = osqpSettings(eps_abs = 1e-8, eps_rel = 1e-9, max_iter = 2e5, verbose = FALSE))
    finalsolution = core_casanovafit(casmodel, y, xlinear, xshrink, L, 2, AIC, mBICconstant, lambdaseq, BICearlystop, BICterminate )
    rm(casmodel)
    ...
    L = CASANOVA_objectmaker(y, xlinear, xshrink, adaptive, prev.coefficients)
    casmodel = osqp(L$Dmat, L$dvec, L$Amat, L$lvec, L$uvec, pars = osqpSettings(eps_abs = 1e-8, eps_rel = 1e-9, max_iter = 2e5, verbose = FALSE))
    solution = core_casanovafit(casmodel, y, xlinear, xshrink, L, 1, AIC, mBICconstant, lambdaseq, BICearlystop, BICterminate )
